...
public class ArchonStrategy {
    ...
    static void runArchon(RobotController rc) throws GameActionException {
        int archonsLabeled = rc.readSharedArray(GameConstants.SHARED_ARRAY_LENGTH - 1);
        if (archonsLabeled < rc.getArchonCount()) {
            archonID = archonsLabeled;
            rc.writeSharedArray(GameConstants.SHARED_ARRAY_LENGTH - 1, archonsLabeled + 1);
            System.out.println("archonID: " + archonID);
        }
        ...
        if (rc.getRoundNum() % rc.getArchonCount() == archonID) {
            ...
            } else if (miners < soldiers * 7 / 10 && rc.getTeamLeadAmount(rc.getTeam()) < 5000) {
                buildTowardsLowRubble(rc, RobotType.MINER);
